...
class KC(Strategy):
    ...
    def check_indicators(self, ohlc, time):
        ...
        if not self.up_crossed and adx[i] >= trend_level and ma2[i] - ma1[i] >= ma2[i - 1] - ma1[i - 1]:
            if close[i] > kch[i] >= open[i]:
                self.message = 'buy detected'
                self.side = 1
                return 1

        if not self.down_crossed and adx[i] >= trend_level and ma2[i] - ma1[i] <= ma2[i - 1] - ma1[i - 1]:
            if close[i] < kcl[i] <= open[i]:
                self.message = 'sell detected'
                self.side = -1
                return -1

        if not self.up_crossed and adx[i - 1] >= trend_level and ma2[i - 1] - ma1[i - 1] >= ma2[i - 2] - ma1[i - 2]:
            if close[i - 1] > kch[i - 1] >= open[i - 1]:
                self.up_crossed = True

        if not self.down_crossed and adx[i - 1] >= trend_level and ma2[i - 1] - ma1[i - 1] <= ma2[i - 2] - ma1[i - 2]:
            if close[i - 1] < kcl[i - 1] <= open[i - 1]:
