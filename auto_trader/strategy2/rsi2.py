...
class RSI(Strategy):
    ...
    def check_indicators(self, ohlc, time):
        ...
        if close[i] > ma1[i] and ma3[i] > ma2[i] > ma1[i] and up_trend and not down_trend:
            self.trend = 1
        elif close[i] < ma1[i] and ma3[i] < ma2[i] < ma1[i] and down_trend and not up_trend:
            self.trend = -1
        else:
            self.trend = 0
