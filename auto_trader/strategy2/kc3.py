...
class KC(Strategy):
    ...
    def check_indicators(self, ohlc, time):
        ...
        if not self.up_crossed and adx[i] >= trend_level and ma5[i] - ma4[i] >= ma5[i - 1] - ma4[i - 1]\
                and ma3[i] > ma2[i] > ma1[i]:
            if close[i] > kch[i] >= open[i]:
                self.message = 'buy detected'
                self.side = 1
                return 1

        if not self.down_crossed and adx[i] >= trend_level and ma5[i] - ma4[i] <= ma5[i - 1] - ma4[i - 1]\
                and ma3[i] < ma2[i] < ma1[i]:
