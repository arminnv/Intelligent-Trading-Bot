...
from indicators2 import rsi, sma, swing, adx, dc
...
class RSI(Strategy):
    ...
    def check_indicators(self, ohlc, time):
        ...
        if close[i] > ma1[i] and ma3[i] > ma1[i] and up_trend:
            self.trend = 1
        elif close[i] < ma1[i] and ma3[i] < ma1[i] and down_trend:
            self.trend = -1
        else:
            self.trend = 0
        ...
        if self.trend == 1 and not self.up_crossed:
            if close[i] > open[i] and rsi[i] > sma[i] and rsi[i - 1] <= sma[i - 1] and check_cross(ohlc, 1)\
                    and max(high[-30:-1]) > close[i]:
                self.message = 'buy detected'
                self.side = 1
                return 1
            elif close[i - 1] > open[i - 1] and rsi[i - 1] > sma[i - 1] and rsi[i - 2] <= sma[i - 2]\
                    and check_cross(ohlc.iloc[:-1], 1) and max(high[-31:-2]) > close[i - 1]:
                self.up_crossed = True

        elif self.trend == -1 and not self.down_crossed:
            if close[i] < open[i] and rsi[i] < sma[i] and rsi[i - 1] >= sma[i - 1] and check_cross(ohlc, -1)\
                    and min(low[-30:-1]) < close[i]:
                self.message = 'sell detected'
                self.side = -1
                return -1
            elif close[i - 1] < open[i - 1] and rsi[i - 1] < sma[i - 1] and rsi[i - 2] >= sma[i - 2] \
                    and check_cross(ohlc.iloc[:-1], -1) and min(low[-30:-2]) < close[i - 1]:
                self.down_crossed = True
