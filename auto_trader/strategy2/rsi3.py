...
class RSI(Strategy):
    ...
    def check_indicators(self, ohlc, time):
        ...
        if close[i] > ma1[i] and ma3[i] > ma2[i] > ma1[i] and up_trend and not down_trend and adx[i] >= trend_level:
            self.trend = 1
        elif close[i] < ma1[i] and ma3[i] < ma2[i] < ma1[i] and down_trend and not up_trend and adx[i] >= trend_level:
            self.trend = -1
        else:
            self.trend = 0
        ...
        if self.trend == 1 and not self.up_crossed:
            if close[i] > open[i] and rsi[i] > sma[i] and rsi[i - 1] <= sma[i - 1] and self.over_bs == 1:
