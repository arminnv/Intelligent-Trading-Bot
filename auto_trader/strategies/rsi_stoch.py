...
from indicators2 import rsi, dc, sma, stoch
...
class RSDC(Strategy):
    ...
    def add_indicators(self, ohlc):
        ...
        ohlc['k'], ohlc['d'] = stoch(ohlc, k_period=self.periods['k'], smooth=self.periods['smooth'],
                                     d_period=self.periods['d'])
    ...
    def check(self, ohlc, time):
        ...
        if side != 0:
            if (ohlc['close'][time] - ohlc['open'][time]) * side > 0 and (ohlc['rsi'][time] - ohlc['sma'][time]) * side > 0:
                ...
                if self.interval - 1 <= int(utc_time[14:16]) % self.interval and int(utc_time[17:19]) >= 15:
                    return True
        return False
